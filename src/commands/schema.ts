...
export default class Schema extends APICommand {
    ...
    public async run(): Promise<JsonObj | void> {
        ...
        if (flags.get) {
            ...
            if (!getRequest.ok) {
                return this.err(
                    `Failed to get schema: ${getRequest.status} | ${JSON.stringify(getRequest.error)}`,
                    { name: args.name, phase: 'get', serverError: getRequest.error }
                )
            }
        ...
        if (flags['set-zod']) {
            ...
            if (!createRequest.ok) {
                ...
                return this.err(
                    `Failed to create schema: ${createRequest.status} | ${JSON.stringify(createRequest.error)}`,
                    { name: args.name, phase: 'creation', serverError: createRequest.error }
                )
            }
